...
svg* svg_create(int width, int height)
{
    ...
    if(psvg != NULL)
    {
        *psvg = (svg){.svg = NULL, .width = width, .height = height, .finalized = false};
        ...
        appendstringtosvg(psvg, " class='graph' aria-labelledby='title' role='img'>\n");
        appendstringtosvg(psvg, "<style>\n");
        appendstringtosvg(psvg, ".rotate { transform: rotate(-90deg);}\n");
        appendstringtosvg(psvg, "</style>\n");
        return psvg;
    }
    else
...
// FUNCTION svg_text
//----------------------------------------------------------------
void svg_text(svg* psvg, int x, int y, char* fontfamily, int fontsize, char* fill, char* stroke, char* text, char* class)
{
